...
const expressions = {
  ...
  rc_from_vega: {
    ...
    custom: (request) => {
      const searchKey = Object.keys(request.query).find((key) =>
        key.match(/search/i)
      );
      if (!searchKey) {
        return null;
      ...
      if (
        ...
        !searchValue[0] ||
        typeof searchValue[0] !== "string"
      ) {
        return null;
      }
      return searchValue[0].match(
        /t:\((.*)\)(?:%20|\s*)and(?:%20|\s*)a:\((.*)\)/i
      );
    },
    handler: (match) =>
      `${process.env.RC_BASE_URL}/search?contributor=${match[2]}&title=${match[1]}`,
  },
  ...
  oclc: {
    ...
    handler: async (match) => {
      ...
      const resp = await client.get(
        `/bibs?nyplSource=sierra-nypl&controlNumber=${oclcNum}`
      );
      const id = resp && resp.data && resp.data[0] && resp.data[0].id;
      const varFields =
        resp && resp.data && resp.data[0] && resp.data[0].varFields;
      const field910a =
        varFields &&
        varFields.find(
          (field) =>
      ...
      const isResearch =
        field910a &&
        field910a.subfields.some(
          (subfield) => subfield.tag === "a" && subfield.content === "RL"
        );
  ...
  issn: {
    ...
    handler: (match) =>
      `${process.env.RC_BASE_URL}/search?issn=${match[1]}&redirectOnMatch=true`,
  },

  isbn: {
    expr: /\/search\/i(\w+)/,
    handler: (match) =>
      `${process.env.RC_BASE_URL}/search?isbn=${match[1]}&redirectOnMatch=true`,
  },
  ...
  searchRegWith: {
    ...
    handler: (match) =>
      `${process.env.RC_BASE_URL}/search?q=${recodeSearchQuery(
        match[3]
      )}${getIndexMapping(match[2])}`,
  },
  ...
  searchRegWithout: {
    ...
    handler: (match, request) => {
      ...
      }/search?q=${mappedQuery}${getIndexMapping(match[3])}`;
    },
  },
  ...
  recordReg: {
    ...
    handler: (match, request) => {
      ...
      if (Array.isArray(collection) && collection.includes("circ")) {
        return `${process.env.VEGA_HOST}/search/card?recordId=${bnum.replace(
          /\D/g,
          ""
        )}`;
      }
  ...
  legacyReg: {
    ...
    handler: (match, request) => {
      return `${process.env.WEBPAC_HOST}${match.input}${reconstructQuery(
        request.query
      )}`;
    },
  },
...
module.exports.redirectUrl = async (request) => {
  logger.debug(
    `LegacyCatalogHandler::redirectUrl: Finding match for ${
      request.path
    } among ${Object.keys(expressions).length} expressions`
  );
  ...
  if (
    ...
    url +=
      (url.includes("?") ? "&" : "?") +
      "originalUrl=" +
      reconstructOriginalURL(request);
  }
  return url;
};
